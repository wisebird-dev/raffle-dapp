...
contract RaffleContract is Ownable, usingProvable {
    ...
    //* Mappings */
    mapping (bytes32 => Raffle) public betting;            
    ...
    function __callback(bytes32 _queryId, string memory _result, bytes memory _proof) public {
        ...
        uint256 randomNumber = uint256(keccak256(abi.encodePacked(_result))) % 2; //change to % numberOfParticipants
        ...
        }else if(now - betting[_queryId].timePlaced > 600000) {
    ...
    function flip() public payable costs(0.01 ether){
        ...
        betting[queryId] = Raffle({player: msg.sender, value: msg.value, result: false, timePlaced: queryTime});
